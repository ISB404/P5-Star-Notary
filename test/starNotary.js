...
contract('StarNotary', async (accs) => {
  ...
  it('can Create a Star', async() => {
    ...
    it('can get a Star Name', async() => {
    let tokenId = 1;
    let StarName;
    StarName = await instance.lookUptokenIdToStarInfo(tokenId, {from: accounts[0]})
  ...
  it('token name and token symbol are added properly', async() => {
    assert.equal(await instance.name.call(), 'P5 Star Notary')
    assert.equal(await instance.symbol.call(), 'P5SN')
  });
  ...
  it('lets 2 users can exchange their stars', async() => {
    ...
    await instance.exchangeStars(starId1, user2, {from: user1})
    await instance.exchangeStars(starId2, user1, {from: user2})
